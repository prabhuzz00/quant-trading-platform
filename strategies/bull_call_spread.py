...
class BullCallSpread(BaseStrategy):
    """
    Bull Call Spread: Buy lower strike CE (ATM), Sell higher strike CE (ATM + spread_points).
    SL: full debit paid. Target: full width of spread minus debit.
    """
    ...
    def __init__(
        ...
        self._buy_ce_instrument: Optional[Dict] = None
        self._sell_ce_instrument: Optional[Dict] = None
    ...
    def get_instruments_to_subscribe(self) -> List[Dict[str, Any]]:
        instruments = []
        if self._buy_ce_instrument:
            instruments.append(self._buy_ce_instrument)
        if self._sell_ce_instrument:
            instruments.append(self._sell_ce_instrument)
        return instruments
    ...
    async def _load_instruments(self, spot_price: float):
        ...
        higher_strike = self.instrument_manager.get_otm_call_strike(spot_price, self.spread_points)
        self._buy_ce_instrument = await self.instrument_manager.get_option_instrument(
            self.symbol, expiry, "CE", atm, self.exchange_segment, "XX"
        )
        self._sell_ce_instrument = await self.instrument_manager.get_option_instrument(
            self.symbol, expiry, "CE", higher_strike, self.exchange_segment, "XX"
        )
    ...
    async def on_tick(self, data: Dict[str, Any]) -> List[Signal]:
        ...
        if not self._buy_ce_instrument or not self._sell_ce_instrument:
            await self._load_instruments(float(spot_price))
            return []

        signals = []
        buy_ltp = self.instrument_manager.get_ltp(
            self._buy_ce_instrument.get("ExchangeInstrumentID", 0)
        ) if self.instrument_manager else None
        sell_ltp = self.instrument_manager.get_ltp(
            self._sell_ce_instrument.get("ExchangeInstrumentID", 0)
        ) if self.instrument_manager else None
        ...
        if buy_ltp and sell_ltp:
            ...
            signals.append(Signal(
                ...
                exchange_instrument_id=self._buy_ce_instrument.get("ExchangeInstrumentID", 0),
                symbol=f"{self.symbol}_ATM_CE_BUY",
                quantity=self.quantity,
                order_mode=OrderMode.BRACKET,
                limit_price=buy_ltp,
                stoploss_points=round(debit, 2),
                target_points=round(target, 2),
                reason="Bull Call Spread buy leg",
            ))
            signals.append(Signal(
                strategy_name=self.name,
                action=SignalAction.SELL,
                exchange_segment=self.exchange_segment,
                exchange_instrument_id=self._sell_ce_instrument.get("ExchangeInstrumentID", 0),
                symbol=f"{self.symbol}_OTM_CE_SELL",
                quantity=self.quantity,
                order_mode=OrderMode.BRACKET,
                limit_price=sell_ltp,
                stoploss_points=round(sell_ltp * 2, 2),
                target_points=round(sell_ltp, 2),
                reason="Bull Call Spread sell leg",
            ))
            self._position_open = True
            logger.info("Bull call spread entry signals generated", symbol=self.symbol, debit=debit)
